...
async function fetchNextPageOfStories() {
    ...
    let endpoint = activePathS3["stories-gyan"].replace(".json", "");
    let url = `${common.API_BASE_URL}/${endpoint}`;
    if (lastKey) {
        url += `?lastKey=${encodeURIComponent(lastKey)}`;
    }
    
    console.log("gyan.js: Fetching URL:", url);
    
    try {
        const response = await fetch(url);
        console.log("gyan.js: Fetch response status:", response.status);
        const data = await response.json();
        console.log("gyan.js: Data received:", data);
        ...
        console.log(`gyan.js: Appended ${fetchedStories.length} stories. Total: ${GlobalJSONObj.length}. hasMore: ${hasMore}, next lastKey: ${lastKey}`);
        ...
        console.error("gyan.js: Error fetching next page:", err);
    } finally {
        isFetching = false;
    }
}
...
async function deleteStory(slug) {
    if (confirm("Are you sure you want to delete this?")) {
        ...
        try {
            ...
            console.log("Story removed from Gyan successfully via API.");
        } catch (error) {
            console.error("Error deleting story from Gyan:", error);
...
async function saveUPre() {
    ...
    $('body').toggleClass('loaded');
    var meta = await WriteS3Bucket(GlobalJSONObj, activePathS3["stories-gyan"]);
    $('body').toggleClass('loaded');
    if (meta.err) {
...
const ReOrderStory = async () => {
    ...
    $('body').toggleClass('loaded');
    await WriteS3Bucket(GlobalJSONObj, activePathS3["stories-gyan"]);
    $('body').toggleClass('loaded');
        ...
        for (let i = 0; i < selectedCheckboxes.length; i++) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    console.log(`Deleted ${deleteSlug} from Gyan via API.`);
                }
            } catch (e) {
...
function refreshVisibleStories() {
    console.log("gyan.js: refreshVisibleStories. GlobalJSONObj length:", GlobalJSONObj ? GlobalJSONObj.length : 0);
...
$(window).on("scroll", function () {
    ...
    scrollTimeout = setTimeout(() => {
        ...
        console.log(`gyan.js: Scroll - scrollTop: ${scrollTop}, windowHeight: ${windowHeight}, documentHeight: ${documentHeight}, hasMore: ${hasMore}, isFetching: ${isFetching}`);
        
        if (scrollTop + windowHeight >= documentHeight - 300) {
            if (hasMore && !isFetching) {
                console.log("gyan.js: Triggering fetchNextPageOfStories");
                fetchNextPageOfStories();
            }
        }
    }, 150);
