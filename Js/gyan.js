...
function bloghomeList() {
    ...
    readS3Bucket(activePathS3["stories-gyan"], function (json) {
        if (json.err) {
            $('#divStory').html('');
...
async function saveUPre() {
    ...
    var meta = await WriteS3Bucket(OrderedList, activePathS3["stories-gyan"]);
    $('body').toggleClass('loaded');
    if (meta.err) {
        return console.log(tt.err);
    }
...
$('#btnRemoveSelected').on('click', async function () {
    if (confirm('We will not be able restore! are you sure?')) {
        ...
        $('[name="chkSlug"]:checked').each(async function () {
            ...
            const SlugRawJson = await readS3BucketAsync(activePathS3["stories-gyan"], "");
            if (SlugRawJson.err) {
                ...
                try {
                    ...
                    await WriteS3Bucket(SlugJson, activePathS3["stories-gyan"]);
