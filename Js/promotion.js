...
function bloghomeList() {
    ...
    readS3Bucket(activePathS3["stories-promotion"], function (json) {
        if (json.err) {
            $('#divStory').html('');
...
async function deleteStory(slug) {
    if (confirm("Are you sure you want to delete this?")) {
        ...
        var meta = await WriteS3Bucket(GlobalJSONObj, activePathS3["stories-promotion"]);
        if (meta.err) {
...
async function saveUPre() {
    ...
    var meta = await WriteS3Bucket(OrderedList, activePathS3["stories-promotion"]);
    $('body').toggleClass('loaded');
    if (meta.err) {
        return console.log(tt.err);
    }
...
const ReOrderStory = async () => {
    ...
    await WriteS3Bucket(GlobalJSONObj, activePathS3["stories-promotion"]);
...
$('#btnRemoveSelected').on('click', async function () {
    if (confirm('We will not be able restore! are you sure?')) {
        ...
        $('[name="chkSlug"]:checked').each(async function () {
            ...
            const SlugRawJson = await readS3BucketAsync(activePathS3["stories-promotion"], "");
            if (SlugRawJson.err) {
                ...
                try {
                    ...
                    await WriteS3Bucket(SlugJson, activePathS3["stories-promotion"]);
