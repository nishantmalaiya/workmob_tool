...
function renderData(newRecords) {
    ...
    newRecords.forEach(record => {
        ...
        storyCard += `<div class="col-md-3">
            <a href="#" data-toggle="modal" data-target="#VFLlocationModal" onclick="editLocation('${record.location}', '${record.id}')">Edit</a>
        </div>`;
    });
    ...
    validation(async function (cansave) {
        if (cansave.cansave) {
            ...
            if ($("#hdnLocationId").val() == "") {
                ...
                const IsExists = await existsS3Bucket(`locations/${cansave["item"]["location"].toLowerCase()}.json`);
                if (!IsExists.isExists) {
                    var a = [];
                    await WriteS3Bucket(a, `locations/${cansave["item"]["location"].toLowerCase()}.json`);
                }
            }
            else {
            ...
            $('#VFLlocationModal').find('#txtLocation').val("");
            $('#VFLlocationModal').find('#hdnLocationId').val("");
            ...
            $(vlocationMaster).each(function () {
                storyCard = storyCard + "<div class=\"col-md-9\">" + this.location + "</div>";
                storyCard = storyCard + "<div class=\"col-md-3\"><a href=\"#\" data-toggle=\"modal\" data-target=\"#VFLlocationModal\" onclick=\"editLocation('" + this.location + "','" + this.id + "')\">Edit</a></div>";
            });
            $('#divStory').append(storyCard);
        }
        else {
...
$("#btnAddLocation").click(function () {
    $('#VFLlocationModal #txtLocation');
    $("#hdnLocationId").val('');
    $('#VFLlocationModal').find('.modal-title').text("Add New Location");
});
...
function editLocation(location, id) {
    $('#VFLlocationModal').find('.modal-title').text("Edit Location");
    $('#VFLlocationModal').find('#txtLocation').val(location);
    $('#VFLlocationModal').find('#hdnLocationId').val(id);
}
function validation(cb) {
    var cansave = true;
    var msg = "";
    var item = {
        "id": $('#VFLlocationModal #txtLocation').val().toLowerCase().replace(/ /g, "_"),
        "location": $.trim($('#VFLlocationModal').find('#txtLocation').val())
    };
    if (item["location"] == "") {
