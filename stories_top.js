...
function storiestopList() {
    ...
    const url = "https://r5dojmizdd.execute-api.ap-south-1.amazonaws.com/prod/stories-top?limit=10";
    fetch(url)
...
$('#btnRemoveSelected').on('click', async function () {
    if (confirm('We will not be able restore! are you sure?')) {
        ...
        $('[name="chkSlug"]:checked').each(async function () {
            ...
            const SlugRawJson = await readS3BucketAsync(activePathS3["stories-top"], "");
            if (SlugRawJson.err) {
                ...
                try {
                    ...
                    await WriteS3Bucket(SlugJson, activePathS3["stories-top"]);
